...
不要上传 `release/`、`backups/`、`node_modules/`、本机设置、API Key、生成历史、用户音频、日志、个人路径或签名证书。
...
5. 上传完成后检查目录层级，特别是 `.github/workflows/` 和 `docs/`。
...
仓库中的 **Build macOS Preview** 仅供未来适配验证：

1. 打开 **Actions**。
2. 左侧选择 **Build macOS Preview**。
3. 点击 **Run workflow**，分支选择 `main`。
4. 构建成功后可在运行页面下载临时 Artifact。
5. Artifact 只用于真机测试，不加入当前公开 Release。

工作流只允许手动触发，并通过 `--publish never` 明确禁止 electron-builder 自动发布，因此不需要 `GH_TOKEN`。如果未来正式发布 macOS，应先完成真机回归、Developer ID 签名、Apple 公证和文档更新。
