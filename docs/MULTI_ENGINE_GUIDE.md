...
| 火山引擎 | 云端 | V1 HTTP 兼容接口 | 使用公司账号已开通的 voice_type 合成 | 是 |
...
应用不会附带任何模型权重、整合包、公司 Key 或个人 Key。

## 火山引擎

官方产品与能力总览：[豆包语音](https://docs.volcengine.com/docs/6561/163032?lang=zh)。该页面用于确认产品能力范围，不是单一接口的完整请求定义；实际接入仍应以公司账号控制台中已开通产品对应的 API 文档、鉴权字段和资源 ID 为准。
